...
SELECT CONCAT( "$", FORMAT(SUM(Salary),2)) AS TotalSalary FROM Employees;
...
SELECT MAX(UnitPrice) AS MostExpensiveItem FROM Products;
...
-- product name.
-- FORMAT turns numerical values to strign makes order by act weird 
